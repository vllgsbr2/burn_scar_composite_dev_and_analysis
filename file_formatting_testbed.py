def make_gis_rdy_file(daily_compiste_file=None, final_product_save_path=None):
    ...
    # and database file
    if daily_compiste_file==None:
        analysis_year        = '2020'
        home                 = '/scratch/zt1/project/vllgsbr2-prj/'
        home_data            = home      + 'raw_data_burnscar/data/'
        daily_composite_file = home_data + f'daily_dlcf_rgb_composites/{analysis_year}_daily_DLCF_RGB_composites.h5'
        commongrid_file      = home_data + 'grids/Grids_West_CONUS_new.h5'
        final_product_file   = home_data + 'final_product/pbsm/primitive_burn_scar_mask.nc'
    else:

        daily_composite_file = daily_composite_file
        final_product_file = final_product_save_path

    with h5py.File(daily_composite_file, 'r') as hf_database:
        ...
        for i, time_stamp in enumerate(list(hf_database.keys())):
            ...
            with Dataset(final_product_file, 'w', format='NETCDF4_CLASSIC') as nc_burnscar,\
                 h5py.File(commongrid_file,'r') as h5_lat_lon:
                ...
                lat           = nc_burnscar.createVariable( 'lat_grid', np.float32,\
                                                           ('lat','lon'), fill_value=-999)
                lat.units     = 'degrees_north'
                lat.long_name = 'latitude'
                lon           = nc_burnscar.createVariable( 'lon_grid', np.float32,\
                                                           ('lat','lon'), fill_value=-999)
                ...
                pbsm               = nc_burnscar.createVariable('pbsm', np.float64,\
                                                                ('time','lat','lon'),\
                                                                fill_value=-999) # note: unlimited dimension is leftmost
                pbsm.units         = 'unitless'
                pbsm.standard_name = 'primitive burn scar mask'
                pbsm[:,:]          = burnscar_mask[r1:r2,c1:c2].reshape((1,pbsm_shape[0],\
                                                                           pbsm_shape[1]))

                day_land_cloud_fire_RGB               = nc_burnscar.createVariable('day_land_cloud_fire_RGB',\
                                                         np.float64,('time','lat','lon','channel'),\
                                                         fill_value=-999) # note: unlimited dimension is leftmost
                day_land_cloud_fire_RGB.units         = 'unitless'
                day_land_cloud_fire_RGB.standard_name = 'day_land_cloud_fire_RGB'
                day_land_cloud_fire_RGB[:,:,:]        = np.copy(X)[r1:r2,c1:c2,:]\
                                                                   .reshape((1,pbsm_shape[0], pbsm_shape[1], 3))
