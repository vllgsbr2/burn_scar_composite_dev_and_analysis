...
from burn_scar_composites import get_burn_scar_composite,\
                                 ...
                                 get_soil_adjusted_vegetation_index
'''
...
def train_GMM(n_clusters = 5):
    ...
    #remove config 
    home_dir = "/scratch/zt1/project/vllgsbr2-prj/burnscar_project/"
    #Javier needs to give me file 
    databse_name = 'data/daily_DLCF_composites_2021_west_CONUS_all_days.h5'
    ...
    NDVI             = get_normalized_differenced_vegetation_index(Ref_0670nm, Ref_0860nm)
    NBR              = get_normalized_burn_ratio(Ref_0860nm, Ref_2250nm)
    SAVI             = get_soil_adjusted_vegetation_index(Ref_0860nm, Ref_0670nm)
    ...
    NDVI       = NDVI[np.isnan(NDVI)==False]
    NBR        = NBR[np.isnan(NBR)==False]
    SAVI       = SAVI[np.isnan(SAVI)==False]
    X = np.concatenate((Ref_2250nm.reshape(-1,1),\
                        Ref_0860nm.reshape(-1,1),\
                        Ref_0670nm.reshape(-1,1),\
                        NDVI.reshape(-1,1),\
                        NBR.reshape(-1,1),\
                        SAVI.reshape(-1,1)), axis=1)
    ...
    inputs = {'R_2250': 1, 'R_0860': 1, 'R_0670': 1, 'VZA': 0,'cosSZA': 0,\
              'RAA'   : 0, 'NDVI'  : 1, 'NBR'   : 1, 'pbsm': 0, 'SAVI': 1}
    ...
    results_path = home_dir + 'data/results/GMM_results/ref_ndvi_nbr/'
    with h5py.File(results_path + results_filename, 'w') as file:
...
def plot_segmented_classified_images_for_analysis(DLCF_RGB, labels, n_clusters=5, fname=None):
    ...
    labeled_DLCF_RGB[np.where(np.isnan(DLCF_RGB[:,:,0])==True)] = np.nan
...
if __name__ == "__main__":
    # read in file
    home_path = '/scratch/zt1/project/vllgsbr2-prj/burnscar_project/'
    #results_files  = np.sort(os.listdir(home_path+'data/results/GMM_results/ref_ndvi_nbr/'))
    #results_file   = [home_path + 'data/results/GMM_results/ref_ndvi_nbr/' + x for x in results_files]
    #fig_file_names = [home_path + 'data/figures/GMM_figures/ref_ndvi_nbr/'   + x[:-3] for x in results_files]
    '''
    for i in range(2,15):
    ...
    '''
    result_home_path = '/scratch/zt1/project/vllgsbr2-prj/burnscar_project/data/results/GMM_results/'
    # file the list below with the directoies of each result directory
    # you told me you were gonna make a results directory for each model configuration
    list_of_result_directories = ['3refs/', '3refs_ndvi_nbr_savi/', 'burn_veg/',\
                                  'burn_veg_nbr/', 'nbr_savi/', 'ndvi_nbr_savi/']
    figure_home_path = '/scratch/zt1/project/vllgsbr2-prj/burnscar_project/data/figures/GMM_figures/'

    for result_directory in list_of_result_directories:
        for result_file in os.listdir(result_home_path+result_directory):
            
            fig_file_name = figure_home_path + result_directory + result_file[:-3]
            idx_cluster_start = result_file.find('nClusters_')+10
            idx_cluster_end   = result_file.find('nClusters_')+12
            num_clusters = int(result_file[idx_cluster_start:idx_cluster_end])
            print(num_clusters,fig_file_name)
            '''
            ...
            labels      ,\
            X           ,\
            DLCF_RGB    = open_GMM_results_file(result_home_path + result_directory + result_file)
