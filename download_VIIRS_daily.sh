...
do
  for f in $(find "$target_directory_VJ109/VJ109/$year/$day_of_year/" -name "VJ109.A*.$hour*"); do rm $f; done
done

# delete extra directories from download after moving files up 3 dir
mv $target_directory_VJ109/VJ109_NRT/$year/$day_of_year/* ../../../
...
for hour in "${invalid_hours_list[@]}"
do
  for f in $(find "$target_directory_VJ103/VJ103MOD_NRT/$year/$day_of_year/" -name "VJ103MOD.A*.$hour*"); do rm $f; done
done

# delete extra directories from download after moving files up 3 dir
mv $target_directory_VJ103/VJ103MOD_NRT/$year/$day_of_year/* ../../../
